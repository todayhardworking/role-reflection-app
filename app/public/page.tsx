...
export default function PublicPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await fetch("/api/publicReflections");
        if (!res.ok) throw new Error("Failed to load");
        const data = await res.json();
        setReflections(data.reflections || []);
      } catch (err) {
        console.error("Error loading public reflections:", err);
        setError("Unable to load public reflections right now");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <section className="space-y-4 text-center">
        ...
        <p className="text-sm text-slate-600">Loading public reflections...</p>
      </section>
    );
  }
